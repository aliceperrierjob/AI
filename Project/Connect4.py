...
def max_value_ab(board, turn, alpha, beta, ai_level):
    if board.check_victory():
        return -1000
    if turn > ai_level:
        return (-1)*board.eval(turn % 2 + 1)
    possible_moves = board.get_possible_moves()
    value = -10000
    for move in possible_moves:
        updated_board = board.copy()
        updated_board.add_disk(move, turn % 2 + 1, update_display=False)
        value = max(value, min_value_ab(updated_board, turn + 1, alpha, beta,ai_level))
        if value >= beta:
            return value
...
def min_value_ab(board, turn, alpha, beta, ai_level):
    ...
    for move in possible_moves:
        updated_board = board.copy()
        updated_board.add_disk(move, turn % 2 + 1, update_display=False)
        value = min(value, max_value_ab(updated_board, turn + 1, alpha, beta, ai_level))
        if value <= alpha:
            return value
        beta = min(beta, value)
    return value
    ...
    def eval(self, player):
        ...
        for line in range(6):
            for horizontal_shift in range(4):
                if self.grid[horizontal_shift][line] == self.grid[horizontal_shift + 1][line] == self.grid[horizontal_shift + 2][line] == player:
                    if self.grid[horizontal_shift+3][line] == 0:
                        r +=100
                    else : 
                        r += 0
                elif self.grid[horizontal_shift][line] == self.grid[horizontal_shift + 1][line] == player:
                    if self.grid[horizontal_shift+2][line] == 0:
                        r +=10
                    else : 
                        r += 0
        ...
        for column in range(7):
            for vertical_shift in range(3):
                if self.grid[column][vertical_shift] == self.grid[column][vertical_shift + 1] == self.grid[column][vertical_shift + 2] == player:
                    if self.grid[column][vertical_shift +3] == 0:
                        r += 100
                    else :
                        r += 0
                elif self.grid[column][vertical_shift] == self.grid[column][vertical_shift + 1] == player:
                    if self.grid[column][vertical_shift +2] == 0:
                        r += 10
                    else :
                        r += 0
        ...
        for horizontal_shift in range(4):
            for vertical_shift in range(3):
                if self.grid[horizontal_shift][vertical_shift] == self.grid[horizontal_shift + 1][vertical_shift + 1] == self.grid[horizontal_shift + 2][vertical_shift + 2] == player :
                    if self.grid[horizontal_shift + 3][vertical_shift + 3] == 0:
                        r +=100
                elif self.grid[horizontal_shift][vertical_shift] == self.grid[horizontal_shift + 1][vertical_shift + 1] == player : 
                    if self.grid[horizontal_shift + 2][vertical_shift + 2] ==0 :
                        r +=10
                    else : 
                        r+=0
                elif self.grid[horizontal_shift][5 - vertical_shift] == self.grid[horizontal_shift + 1][4 - vertical_shift]== self.grid[horizontal_shift + 2][3 - vertical_shift] == player:
                    if self.grid[horizontal_shift + 3][2 - vertical_shift] == 0 :
                        r += 100
                    else : 
                        r += 10
                elif self.grid[horizontal_shift][5 - vertical_shift] == self.grid[horizontal_shift + 1][4 - vertical_shift] == player :
                    if self.grid[horizontal_shift + 2][3 - vertical_shift] == 0 : 
                        r +=10
                    else :
                        r += 0
        return r
